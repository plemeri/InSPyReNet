...
class EInSPyReNet_SwinB(nn.Module):
    ...
    def __init__(self, channels=64, pretrained=True):
        ...
        self.attention =  ASCA_DWT(channels    , channels)#, lmap_in=True)
        self.attention1 = ASCA_DWT(channels * 2, channels)#, lmap_in=True)
        self.attention2 = ASCA_DWT(channels * 2, channels)
    ...
    def forward(self, x, y=None):
        ...
        f2, l2 = self.attention2(torch.cat([x2, self.ret(f3, x2)], dim=1), d3.detach()) # 8
        d2 = self.dwt_inverse((d3.detach(), [l2.unsqueeze(1)])) # 4
        # d2 = self.inspyre.rec(d3.detach(), l2) # 4h 4w

        f1, l1 = self.attention1(torch.cat([x1, self.ret(f2, x1)], dim=1), d2.detach()) #, l2.detach()) # 4
        # d1 = self.inspyre.rec(d2.detach(), l1) # 8h 8w
        d1 = self.dwt_inverse((d2.detach(), [l1.unsqueeze(1)])) # 2

        _, l = self.attention(self.res(f1, (H // 2, W // 2)), d1.detach()) #, l1.detach()) # 2
        d = self.dwt_inverse((d1.detach(), [l.unsqueeze(1)])) # 1
        ...
        if y is not None:       
            ...
            y1, [ly1] = self.dwt_forward(y)
            y2, [ly2] = self.dwt_forward(y1)
            y3, [ly3] = self.dwt_forward(y2)
            ...
            closs =  self.loss_fn(d3, y3 // (2 ** 3))
            closs += self.loss_fn(d2, y2 // (2 ** 2))
            closs += self.loss_fn(d1, y1 // (2 ** 1))
            closs += self.loss_fn(d,  y)
            
            loss = closs #ploss1 + ploss2 + ploss3 + closs
