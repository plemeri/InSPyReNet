...
class InSPyReNet_DH(nn.Module):
    ...
    def forward(self, sample):
        ...
        x5, x4, x3, x2, x1 = out['backbone_feats']
        f3, f2, f1 = out['feats']
        p2, p1, p0 = out['laplacian']
        d3, d2, d1, d0 = out['gaussian']
        ...
        x5, x4, x3, x2, x1 = torch.cat([x5, dx5], dim=1), torch.cat([x4, dx4], dim=1), torch.cat([x3, dx3], dim=1), torch.cat([x2, dx2], dim=1), torch.cat([x1, dx1], dim=1)
        f3, f2, f1 = torch.cat([f3, df3], dim=1), torch.cat([f2, df2], dim=1), torch.cat([f1, df1], dim=1)
