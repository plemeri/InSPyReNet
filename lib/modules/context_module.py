...
class PAA_kernel2(nn.Module):
    def __init__(self, in_channel, out_channel, receptive_size, stage_size=None):
        super(PAA_kernel2, self).__init__()
        self.conv0 = conv(in_channel, out_channel, 1)
        self.conv1 = conv(out_channel, out_channel, kernel_size=(1, receptive_size))
        self.conv2 = conv(out_channel, out_channel, kernel_size=(receptive_size, 1))
        self.conv3 = conv(out_channel, out_channel, 3, dilation=receptive_size)
        self.Hattn = self_attn2(out_channel, 'h', stage_size[0] if stage_size is not None else None)
        self.Wattn = self_attn2(out_channel, 'w', stage_size[1] if stage_size is not None else None)
    ...
    def forward(self, x):
        ...
        x = self.conv3(Hx + Wx)
        return x
class PAA_e2(nn.Module):
    def __init__(self, in_channel, out_channel, base_size=None, stage=None):
        super(PAA_e2, self).__init__()
        self.relu = nn.ReLU(True)
        if base_size is not None and stage is not None:
            self.stage_size = (base_size[0] // (2 ** stage), base_size[1] // (2 ** stage))
        else:
        ...
        self.branch0 = conv(in_channel, out_channel, 1)
        self.branch1 = PAA_kernel2(in_channel, out_channel, 3, self.stage_size)
        self.branch2 = PAA_kernel2(in_channel, out_channel, 5, self.stage_size)
        self.branch3 = PAA_kernel2(in_channel, out_channel, 7, self.stage_size)
