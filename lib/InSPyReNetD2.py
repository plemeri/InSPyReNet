...
class InSPyReNetD2(nn.Module):
    def __init__(self, backbone, in_channels, depth=64, base_size=384, **kwargs):
        ...
        self.attention2 = ASCA(self.depth * 2, depth, base_size=base_size, stage=2, lmap_in=True)
    ...
    def cuda(self):
        ...
        self = super(InSPyReNetD2, self).cuda()
        return self
    ...
    def forward(self, sample):
        ...
        dx1 = self.pyr.down(sample['depth'])
        ...
        f2, p2 = self.attention2(torch.cat([x2, f3], dim=1), d3.detach(), dx3)
        d2 = self.pyr.rec(d3.detach(), p2) #4

        x1 = self.res(x1, (H // 2, W // 2))
        f2 = self.res(f2, (H // 2, W // 2))
        f1, p1 = self.attention1(torch.cat([x1, f2], dim=1), d2.detach(), dx2) #2
        d1 = self.pyr.rec(d2.detach(), p1) #2
        
        f1 = self.res(f1, (H, W))
        _, p0 = self.attention0(f1, d1.detach(), dx1) #2
...
def InSPyReNetD2_Res2Net50(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(res2net50_v1b_26w_4s(pretrained=pretrained), [64, 256, 512, 1024, 2048], depth, base_size, **kwargs)

def InSPyReNetD2_Res2Net101(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(res2net101_v1b_26w_4s(pretrained=pretrained), [64, 256, 512, 1024, 2048], depth, base_size, **kwargs)

def InSPyReNetD2_SwinS(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(SwinS(pretrained=pretrained), [96, 96, 192, 384, 768], depth, base_size, **kwargs)

def InSPyReNetD2_SwinT(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(SwinT(pretrained=pretrained), [96, 96, 192, 384, 768], depth, base_size, **kwargs)
    
def InSPyReNetD2_SwinB(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(SwinB(pretrained=pretrained), [128, 128, 256, 512, 1024], depth, base_size, **kwargs)

def InSPyReNetD2_SwinL(depth, pretrained, base_size, **kwargs):
    return InSPyReNetD2(SwinL(pretrained=pretrained), [192, 192, 384, 768, 1536], depth, base_size, **kwargs)